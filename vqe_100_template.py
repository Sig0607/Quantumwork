...
def run_vqe(H):
      ...
      #      variational_ansatz(params, H.wires)
       #     cost_fn = qml.ExpvalCost(variational_ansatz, H, dev)
        #    energy = cost_fn(params)
         #   qml.GradientDescentOptimizer(stepsize=0.002)
          #  return energy
    
   # @qml.qnode(dev, diff_method="parameter-shift")
    #def circuit():
     #   qml.BasisState(params, requires_grade=False, wires=num_qubits)
   ...
   # for i in num_qubits:
    ...
    #     qml.broadcast(qml.CNOT, wires=[0, 1, 2], pattern="ring")
    #     return qml.expval(qml.PauliY(0) @ qml.PauliZ(0))
    #@qml.qnode(dev, diff_method="parameter-shift")
    #def circuit():
       ...
       # qml.broadcast(qml.CNOT, wires=[0, 1, 2], pattern="ring")
        ...
        # This under gives errors sooooooo
        #qml.RX(params[0][3], wires=0)
        #qml.RY(params[0][4], wires=1)
        #qml.RZ(params[0][5], wires=2)

        #qml.broadcast(qml.CNOT, wires=[0, 1, 2], pattern="ring")
        #return qml.expval(qml.PauliY(0) @ qml.PauliZ(2))

    #cost_fn = qml.ExpvalCost(circuit, H, dev)
    #print(circuit(), cost_fn)
